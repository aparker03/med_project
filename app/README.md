...
This folder contains the Streamlit app for the Healthcare Access and Cost Burden Explorer. The app turns saved CSV outputs into a readable project workspace for reviewing weighted NHIS summaries, model artifacts, limitations, and variable references.
...
pages/2_MEPS_Explorer.py
  Preview page for app-ready processed or interim MEPS CSV files

pages/3_Model_Lab.py
  Saved model comparisons, feature importance, threshold tradeoffs, lift review, and metric explanations
...
The MEPS page looks for app-ready MEPS CSVs in `data/processed` or `data/interim`. It intentionally avoids `data/raw`.
...
The Model Lab is a decision-support learning tool for reviewing saved model behavior. It is not a clinical tool, eligibility tool, pricing tool, or individual risk labeler.

The saved models estimate whether a row belongs to the measured target class based on patterns in training data. In this project, that target is a cost-related mental health care barrier. The model outputs can help compare tradeoffs such as precision, recall, F1, AUC, average precision, threshold behavior, and lift. They do not identify who truly needs care or who deserves services.

## Future Improvements

Possible next steps include:

- Add processed MEPS summaries once app-ready MEPS CSVs are available
