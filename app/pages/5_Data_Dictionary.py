...
available_columns = list(model_df.columns) if model_df is not None and not model_df.empty else []
...
if model_df is not None and not model_df.empty:
    ...
    columns_df = pd.DataFrame(
        {
            "Plain-language label": [friendly_label(col) for col in model_df.columns],
            "Variable name": list(model_df.columns),
            "Type": [str(dtype) for dtype in model_df.dtypes],
        }
    )
